...
HFS = "20.5.332"  # Default Houdini version (can be changed via args)
...
def ensure_directories_and_symlinks(project_name):
    """Create necessary directories for the project and create a symbolic link named 'SIM' in HOUDINI_FILES_DIR, pointing to SIM in HOUDINI_PROJECT_DIR."""
    required_dirs = ['GEO', 'HDA', 'HIP', 'REF', 'RENDER', 'COMP']  # Directories to be created
    ...
    # Create SIM directory in HOUDINI_PROJECT_DIR under the current project name
    sim_dir_path_project = os.path.join(PROJECT_DIR, project_name, 'SIM')
    ensure_directory_exists(sim_dir_path_project)
    ...
    # Create symbolic link named 'SIM' in HOUDINI_FILES_DIR pointing to SIM in HOUDINI_PROJECT_DIR
    symlink_path = os.path.join(project_path, 'SIM')
    if not os.path.islink(symlink_path):
        os.symlink(sim_dir_path_project, symlink_path)
        print_green(f"Created symbolic link '{symlink_path}' pointing to '{sim_dir_path_project}'")
    else:
...
def interactive_mode():
    """Interactive mode for managing projects, backups, and restores."""
    while True:
        print_yellow("\n--- Interactive Mode ---")
